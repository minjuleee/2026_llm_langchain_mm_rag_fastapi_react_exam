...
# ── Mock LLM: API 키 없이 ChatOpenAI처럼 흉내 내는 함수 ─────────
def mock_llm_fn(prompt_value) -> object:
    ...
    # 핵심 수정:
    # ChatPromptTemplate이 만든 prompt_value에서 메시지 목록을 꺼냄
    ...
    class FakeResponse:
        """
        ChatOpenAI 응답 객체를 흉내 내는 클래스.
        LangChain의 AIMessage처럼 .content 속성을 가지게 함.
        """
...
# 파이프라인:
# prompt → mock_llm → .content 추출 → JSON 파싱
analysis_chain = (
    prompt
