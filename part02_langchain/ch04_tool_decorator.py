...
# random.seed(42)
# 결과가 다르게 나왔으면 한다 => random.seed(42) 주석 처리
...
def mock_llm_fn(prompt_value) -> object:
    ...
    has_night = bool(re.search(r"0[123]:\d{2}", human_content))
...
def filter_danger_frames(frames_json: str) -> str:
    ...
    Returns:
      위험 프레임만 포함한 JSON 문자열
    """
    frames = json.loads(frames_json)
    danger = [f for f in frames if f.get("risk_level") == "위험"]
...
def get_risk_summary(results_json: str) -> str:
    ...
    for r in data:
        ...
        if lvl == "위험":
            summary["위험_프레임_ids"].append(r.get("frame_id"))
...
# Tool 2 호출 - 창고 출입구 카운트
zone_stat = json.loads(
    count_objects_in_zone.invoke({
        "frames_json": frames_json,
        "zone": "창고 출입구"
    })
)
...
print(
    f"   총 탐지: {zone_stat['total_detections']}건 "
