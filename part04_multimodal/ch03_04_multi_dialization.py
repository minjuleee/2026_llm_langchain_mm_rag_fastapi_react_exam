...
# ─── 설정 ─────────────────────────────────────────────────────
MODEL_NAME = "base"
AUDIO_PATH = "./waves/20260526_All_units.wav"
...
def merge_consecutive_segment(segments: list) -> list :
  ...
  # for seg in segments[1:] :
  #   last = merged[-1]
  #   if seg["speaker"] == last["speaker"] and seg["start"] - last["end"] <= 0.8 :
  #     # 같은 화자 + 시간 간격이 0.8초 이내 → 병합
  #     last["end"] = seg["end"]
  #     last["text"] += " " + seg["text"]
  #   else :
  #     merged.append(seg)
...
if not HF_TOKEN:
    raise ValueError(
        "HF_TOKEN이 없습니다. .env 파일에 HF_TOKEN=hf_xxxxx 형태로 넣어주세요."
    )

# Hugging Face 로그인
# token= / use_auth_token= 인자 버전 충돌을 피하기 위해 먼저 로그인 처리
...
correct_timestamps(result["segments"], actual_duration)
...
#
# min_speakers / max_speakers: 화자 수 범위 지정
# ─────────────────────────────────────────────────────────────
print("=" * 60)
print("STEP 2: pyannote 화자 분리")
print("=" * 60)

# 여기서 token=, use_auth_token= 넣지 않음
# 위에서 login() 했기 때문에 인증 정보는 Hugging Face 캐시에서 사용됨
...
# pyannote 버전에 따라 output 형태가 다를 수 있어서 안전 처리
...
merged_segments = merge_consecutive_segment(segments_with_speakers)
