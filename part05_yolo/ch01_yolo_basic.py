...
# model.predict() : 이미지에 대해 객체 탐자를 실행하는 함수
# source : 소스 (탐지할 이미지 소스)
# conf : 신뢰도 기준. (주어진 값 미만인 클래스는 버림)
# save : True일때 탐지 결과가 그려진 이미지를 자동 저장해라
# 반환값 : list[Results]  -> 여러개의 이미지를 분석 시킬 수 있기 때문에 
#        결과 또한 여러개의 Result가 나올 수 있으므로
# results = model.predict("./images/man_walking_with_dog.jpeg", 
#               conf=0.1, save=True)
source = ['./images/bird.jpeg', './images/cat.jpeg']
results = model.predict("./images/man_walking_with_dog.jpeg", 
              conf=0.1, save=True)
...
for i, r in enumerate(results, start=1) :
  if len(r.boxes) > 0 :
    ...
    print(f"{i}번째 탐지 클래스번호 : {cls_id_0} => 이름 {r.names[cls_id_0]}, 신뢰도 : {round(float(box.conf[0]), 2)}")
    print(f"픽셀 좌표 {[(int(v)) for v in box.xyxy[0].tolist()]}")
    print(f"정규화좌표 {[(round(float(v), 3)) for v in box.xywhn[0].tolist()]}")
