...
# 영구 저장하려면 : client = chromadb.PersistentClient(path="./chromadb")
client = chromadb.Client()
# client = chromadb.PersistentClient(path="./chromadb")

# Collection : VectorDB 안의 데이터를 저장하는 단위 (RDB에서의 테이블 개념과 유사)
...
# ChromaDB 에 저장 (RDB 에서의 insert into문)
# text를 자동으로 벡터로 변환 후 저장 한다
collection.add(
...
results = collection.query(
  query_texts=[query],
  n_results=3,  # 유사한 상위 3개 결과 반환
  include=["documents", "metadatas", "distances"]  # 반환할 정보 선택
)
